...
There are two models: One for flicker (1/f) noise and one for white noise.
...
The amplitude drops off with time at the same rate frequency increases, resulting
in a flat spectrum from DC to the final frequency.
Or the frequency that the chirp would arrive at if it were to continue indefinitely.
...
Since the spectra of all chirps are white, their distribution is up for grabs.
However, an exponential distribution similar to that of the pink noise model would
make the most sense from a bandwidth utilization standpoint.
...
processing technologies such as pattern recognition by convolutional neural
