...
It could realize the dream of Ren\'{e} Descartes, the father of modern
science and originator of Cartesian dualism, by instrumenting dualism itself.
...
The signals that encode this information can be demodulated from what would
otherwise be disregarded as noise.
...
\cite{Novak} that is constant-slope (on a log-log plot) at -3dB per octave,
curiously similar to flicker noise power spectral density.
...
which flattens out the slope of the spectrum so it's still white.
That's a more practical way to detect the chirp due to the shortness of the
time it spends in a bandwidth that lends itself to detection.
