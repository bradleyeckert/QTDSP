...
\cite{Novak} that is flat, but slope-dependent on ``warp factor'' $\omega$.
So, a constant amplitude chirp is not useful.
Dropping the amplitude with frequency results in an exponential chirp with 
a power spectrum, dependent on $\omega$, of:
\begin{equation}
...
amplitude dropping with frequency, compatible with the flicker noise model.
