...
This gives a $k$ of approximately $-2$.
$k$ is tweaked to make both $H_X$ and $H_V$ integers.
The frequency shift caused by $H_X$ is related to $\epsilon$.

\begin{equation}
\frac{f}{f_0} = e^{R \cdot H_X/N} = \frac{\epsilon(H_V)}{\epsilon(0)}
\end{equation}

\begin{equation}
e^{R \cdot H_X/N} = e^{k \cdot H_V/N}
\end{equation}

Once $H_V$ is estimated to the nearest integer, $H_X$ can be
re-calculated from Eq. \ref{eq:hv} to give a $k$ nearest to 2.0:
\begin{equation}  \label{eq:hx}
H_X \approx \frac{2 \cdot H_V}{|R|}
\end{equation}

\begin{equation}
k = |R| \cdot \frac{H_X}{H_V}
\end{equation}
