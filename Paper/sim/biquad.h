...
extern void BiQuad_new(const int type, smp_type dbGain, /* gain of filter */
                       const smp_type freq,             /* center frequency */
                       const smp_type srate,            /* sampling rate */
                       const smp_type bandwidth,        /* bandwidth in octaves */
                       biquad* const b);                /* filter structure */
void BiQuad_clear(biquad* const b);                     /* clear filter state */
